...
function renderSvg(){let svg=N('planSvg'),p=getParams(); let W=p.W,H=p.H;
 ...
 for(const row of state.rows){
  ...
  row.tiles.forEach((t,idx)=>{let x=sx(t.a,scaleX,ox), w=(t.b-t.a)*scaleX; let tileNo=idx+1; let selTile=state.selectedTile&&state.selectedTile.row===row.row&&state.selectedTile.index===tileNo; s.push(`<rect x="${x}" y="${yTop}" width="${w}" height="${yBot-yTop}" fill="${t.full?'var(--tile2)':'#f4c6c6'}" stroke="${selTile?'#005bbb':'#77808a'}" stroke-width="${selTile?3:0.8}" opacity=".96" data-tile-row="${row.row}" data-tile-index="${tileNo}" onpointerdown="selectTile(${row.row},${tileNo});event.preventDefault();event.stopPropagation();" onclick="selectTile(${row.row},${tileNo});event.stopPropagation();" onmouseenter="hoverTile(${row.row},${tileNo})" onmouseleave="hoverTile(null,null)" style="cursor:pointer"></rect>`)});
  s.push(`</g>`);
...
function renderTileMeasureLabels(s,scaleX,ox,oy,rw,rh,bottomTickY,bottomAxisY){
 ...
 items.forEach((it,idx)=>{
  ...
  s.push(`<line x1="${x}" y1="${yBase}" x2="${x}" y2="${yLabel-14}" stroke="${stroke}" stroke-width="${selected?3.0-dev:2}" stroke-dasharray="${selected?'':'4 4'}"/>`);
...
function showObstacle(i=null){state.editing=i; let o=i==null?{name:'Komín',type:'komin',x:1000,y:1000,w:800,h:800}:state.obstacles[i]; N('obsName').value=o.name;N('obsType').value=o.type;N('obsX').value=o.x;N('obsY').value=o.y;N('obsW').value=o.w;N('obsH').value=o.h;N('modal').classList.add('show')}
function hideObstacle(){N('modal').classList.remove('show')} function editObstacle(i){showObstacle(i)} function delObstacle(i){state.obstacles.splice(i,1);state.selectedObstacle=null;state.hoverObstacle=null;recalc()}
function saveObstacle(){let o={id:state.editing==null?Date.now():(state.obstacles[state.editing]?.id||Date.now()),name:N('obsName').value||'Překážka',type:N('obsType').value,x:val('obsX'),y:val('obsY'),w:val('obsW'),h:val('obsH')}; if(o.w<=0||o.h<=0){alert('Detail musí mít kladnou šířku i výšku.');return;} let p=getParams(); if(o.x<0||o.y<0||o.x+o.w>p.W||o.y+o.h>p.H){alert('Detail musí být celý uvnitř střešní roviny.');return;} let overlap=findObstacleOverlap(o,state.editing); if(overlap){alert('Detail se překrývá s položkou „'+overlap.obstacle.name+'“. Upravte polohu nebo rozměr – detaily se nesmí navzájem překrývat.');return;} if(state.editing==null)state.obstacles.push(o);else state.obstacles[state.editing]=o; hideObstacle(); recalc()}
